...
function get(options, callback) {
    if (options.uri) {
        ...
        uri = encodeURI('https://' + options.region + '.battle.net' +
            '/api/' + options.game + options.path + options.query);
    }
    ...
    if (exports.privateKey && exports.publicKey) {
        ...
        headers.Authorization = 'BNET ' + exports.publicKey + ':' + signature.digest('base64');
        headers.Date = date;
    }
    ...
    request({ uri: uri, headers: headers }, function(err, res, body) {
        ...
        if (body) {
            try {
                ...
                return callback(new Error('Error parsing response: ' + jsonErr));
            }
        } else {
...
exports.auctiondata = function() {
    ...
    var parse = function(url) {
        get({
            uri: (typeof url === 'object' && url.url ? url.url : url)
        }, callback);
    };
